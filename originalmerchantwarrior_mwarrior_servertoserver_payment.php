	...
	class MerchantWarrior_Mwarrior_ServerToServer_Payment extends Shop_PaymentType
	{
		...
		public function process_payment_form($data, $host_obj, $order, $back_end = false)
		{
			...
			try
			{
				...
				if (!isset($response_fields['responseCode']) || intval($response_fields['responseCode']) < 1)
				{
				...
				if ($status === false)
				{
					if (strpos($response_fields['responseMessage'], "paymentCardExpiry")!== false) {
						
					throw new Phpr_ApplicationException("Payment Processor declined transaction: ".$response_fields['responseMessage']);
					}
